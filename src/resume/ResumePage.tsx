...
import { ResumeSectionKey, TailoredResume } from '../lib/types';
import { PendingResume, applyHidden } from '../lib/tailoredResume';
...
function documentTitle(h: PendingResume): string {
...
export default function ResumePage() {
  const [handoff, setHandoff] = useState<PendingResume | null>(null);
  const [missing, setMissing] = useState(false);
  const [template, setTemplate] = useState<TemplateId>(DEFAULT_TEMPLATE);
  // Estimated printed page count, measured from the rendered content.
  const [pageCount, setPageCount] = useState(1);
  const pageRef = useRef<HTMLElement>(null);
  // `revision` keys the article so every externally-driven content change
  // (panel toggles, AI revisions) remounts it cleanly — React must never
  // reconcile fresh content against a DOM the user has structurally edited
  // (deleted bullets/sections), which crashes the commit. Template switches
  // don't touch the key, so they keep their edit-preserving behavior.
  const [revision, setRevision] = useState(0);

  useEffect(() => {
    (async () => {
      const stored = await chrome.storage.session.get('pendingResume');
      const raw = stored.pendingResume as PendingResume | TailoredResume | undefined;
      if (!raw) {
        setMissing(true);
        return;
      }
      const h: PendingResume =
        'resume' in raw ? (raw as PendingResume) : { resume: raw as TailoredResume, matchStyle: null };
      setHandoff(h);
    ...
    // The side panel's Resume view writes toggles and revisions into the
    // shared session handoff — pick them up live.
    const listener = (changes: Record<string, chrome.storage.StorageChange>, area: string) => {
      if (area === 'session' && changes.pendingResume) {
        const next = changes.pendingResume.newValue as PendingResume | undefined;
        if (next) {
          setMissing(false);
          setHandoff(next);
          setRevision((r) => r + 1);
  ...
  const { matchStyle } = handoff;
  // Render what the user kept — parts toggled off in the side panel stay out.
  const resume = applyHidden(handoff.resume, handoff.hidden);
  const style =
  ...
  return (
    <>
      ...
      {/* Print shell: with @page margins at zero (that's what suppresses
          Chrome's headers/footers), per-page paper margins come from the
          repeating table header/footer spacers — thead/tfoot repeat on every
          printed page. On screen the shell renders as plain blocks. */}
      <table className="print-shell">
        <thead className="sheet-margin">
          <tr>
            <td aria-hidden="true" />
          </tr>
        </thead>
