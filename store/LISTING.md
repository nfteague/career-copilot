...
> • Tailored resumes too. One click builds a one-page, print-ready resume around your strongest evidence for that posting, mirroring its keywords — and flags the requirements your profile doesn't show, so you can fill them in instead of getting screened out.
...
| Screenshot 3 (1280×800) | `store/screenshot-3.png` | Tailored resume + gap-detection story |
...
2. [ ] `npm run package` → upload `release/career-copilot-v<version>.zip` (currently v0.3.0).
3. [ ] Paste listing copy + upload the four images above.
